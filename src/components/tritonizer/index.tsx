...
function Tritonizer() {
	...
	const color_perms = Combinatorics.permutationCombination(color_list).toArray().filter(list => list.length > 1)
	const canvas_array = []
	let id_no = 0
	while (id_no < color_perms.length) {
		canvas_array.push(<Canvas key={id_no} image={image} id={id_no} colorList={color_perms[id_no]}/>)
		id_no++
	}
	return (<ul>
		{canvas_array}
